        ...
        class ObjectTrack
        {
            public:
                ObjectTrack( ) {
                    ...
                    _trace("Initialized track_lock:%p", track_lock_);
                }
                virtual ~ObjectTrack() {
                ...
                virtual bool CallBackIfObjectExists(unsigned long uid, SafeAssignmentCallBack callbackfn, void *lhs, void *rhs) {
                    ...
                    _trace("Waiting for lock uid:%lu track_lock:%p", uid, &track_lock_);
                    pthread_mutex_lock(&track_lock_);
                    _trace("Acquired lock uid:%lu track_lock:%p", uid, &track_lock_);
                    if (allocated_objects_.find(uid) != allocated_objects_.end() &&
                    ...
                    _trace("Released lock uid:%lu track_lock:%p", uid, &track_lock_);
                ...
                virtual void UntrackObject(void *p, unsigned long uid) {
                    ...
                    _trace("Acquired lock uid:%lu track_lock:%p", uid, &track_lock_);
                    allocated_objects_[uid] = NULL;
                    _trace("Untracking object: %lu: %p track_lock:%p", uid, p, &track_lock_);
                    //printf("Untracking object: %lu: %p\n", uid, p);
                    pthread_mutex_unlock(&track_lock_);
                    _trace("Released lock uid:%lu track_lock:%p", uid, &track_lock_);
                    //freed_objects+=1; 
                ...
                virtual void Clear() {
                    ...
                    _trace("Waiting for lock");
                    pthread_mutex_lock(&track_lock_);
                    _trace("Acquired lock ");
