...
int DeviceCUDA::Init() {
  ...
  _cuerror(err);
  for (int i = 0; i < nqueues_; i++) {
    err = ld_->cuStreamCreate(streams_ + i, CU_STREAM_DEFAULT);
    _cuerror(err);
  }
...
int DeviceCUDA::MemD2D(Task *task, BaseMem *mem, void *dst, void *src, size_t size) {
  ...
  if (is_async()) {
      ...
      err = ld_->cudaMemcpyAsync((void *)dst_cumem, (void *)src_cumem, size, cudaMemcpyDeviceToDevice, streams_[q_]);
      _cuerror(err);
...
int DeviceCUDA::MemH2D(Task *task, BaseMem* mem, size_t *off, size_t *host_sizes,  size_t *dev_sizes, size_t elem_size, int dim, size_t size, void* host, const char *tag) {
  ...
  if (dim == 3) {
      ...
      if (!is_async()) {
          ...
          err = ld_->cuMemcpyHtoDAsync(cumem + off[0], host, size, streams_[q_]);
          _cuerror(err);
          if (err != CUDA_SUCCESS) error_occured = true;
      }
  }
  ...
  if (dim == 3) {
    ...
    if (task->uid() == 277) {
      ...
      if (!is_async()) {
          ...
          err = ld_->cuMemcpyDtoHAsync(host, cumem + off[0], size, streams_[q_]);
          _cuerror(err);
          if (err != CUDA_SUCCESS) error_occured = true;
      }
  }
...
int DeviceCUDA::KernelLaunch(Kernel* kernel, int dim, size_t* off, size_t* gws, size_t* lws) {
  ...
  if (!is_async()) {
      ...
      err = ld_->cuLaunchKernel(cukernel, grid[0], grid[1], grid[2], block[0], block[1], block[2], shared_mem_bytes_, streams_[q_], params_, NULL);
      _cuerror(err);
      if (err != CUDA_SUCCESS){
          worker_->platform()->IncrementErrorCount();
          return IRIS_ERROR;
