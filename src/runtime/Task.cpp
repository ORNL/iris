...
void Task::print_incomplete_tasks()
{
  ...
  for (int i = 0; i < ndepends_; i++) {
    printf("      Running dependent task: %d:%ld:%ld:%s Status:%s object exists:%d %p\n", i, depends_uids_[i], depends_[i]->uid(), depends_[i]->name(), depends_[i]->task_status_string(), platform_->task_track().IsObjectExists(depends_[i], depends_uids_[i]), depends_[i]);
  }
}
...
bool Task::Dispatchable() {
  ...
  for (int i = 0; i < ndepends_; i++) {
    if (platform_->task_track().IsObjectExists(depends_[i], depends_uids_[i]) && depends_[i]->status() != IRIS_COMPLETE) return false;
  }
...
void Task::DispatchDependencies() {
  ...
  for (int i = 0; i < ndepends_; i++) {
      _trace("      Dispatch dependencies task: %d:%ld:%ld:%s Status:%s object exists:%d\n", i, depends_uids_[i], depends_[i]->uid(), depends_[i]->name(), depends_[i]->task_status_string(), platform_->task_track().IsObjectExists(depends_[i], depends_uids_[i]));
      if (platform_->task_track().IsObjectExists(depends_[i], depends_uids_[i]) && depends_[i]->status() == IRIS_PENDING) depends_[i]->status_ = IRIS_NONE;
  }
...
void Task::Complete() {
  ...
  if (platform_->release_task_flag()) {
      for (int i = 0; i < ndepends_; i++)
          if (platform_->task_track().IsObjectExists(depends_[i], depends_uids_[i]) && depends_[i]->user()) depends_[i]->Release();
...
void Task::AddDepend(Task* task, unsigned long uid) {
  if (task == NULL) return;
  if (!platform_->task_track().IsObjectExists(task, uid)) 
      return;
  if (depends_ == NULL) {
