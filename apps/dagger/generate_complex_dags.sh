...
[ $? -ne 0 ] && exit
cat graph.json
for POLICY in random # roundrobin #any
do
...
python ./gantt/gantt.py --dag=./graph.json --timeline=./timeline_log.csv --combined-out=shared-linear.pdf --title-string="Shared Memory on Linear-10 dataset with any scheduling policy" #--drop="Init"
[ $? -ne 0 ] && exit
echo "*******************************************************************"
...
[ $? -ne 0 ] && exit
cat graph.json
for POLICY in any #roundrobin
do
  echo "Running IRIS with Policy: $POLICY"
  IRIS_HISTORY=1 IRIS_HISTORY_FILE=timeline_log.csv ./dagger_runner --graph="graph.json" --logfile="time.csv" --repeats=1 --scheduling-policy="$POLICY" --size=256  --kernels="bigk,process,ijk" --duplicates="0" --buffers-per-kernel="bigk:rw r r,process:rw,ijk:w r r" --kernel-dimensions="bigk:2,process:1,ijk:2" --kernel-split='60,20,20' --depth=10 --num-tasks=10 --min-width=1 --max-width=1 --use-data-memory
    [ $? -ne 0 ] && exit
done
python ./gantt/gantt.py --dag=./graph.json --timeline=./timeline_log.csv --combined-out=linear-dmem.pdf --title-string="IRIS Data-Memory on Linear-10 dataset with any scheduling policy" #--drop="Init"
[ $? -ne 0 ] && exit
echo "*******************************************************************"
echo "*                             Linear-10                           *"
echo "*******************************************************************"
...
python ./gantt/gantt.py --dag=./graph.json --timeline=./timeline_log.csv --combined-out=linear.pdf --title-string="Linear-10 dataset with any scheduling policy" #--drop="Init"
[ $? -ne 0 ] && exit
