...
def go_quantum(iris_params, iris_dev):
    ...
    backend = AerSimulator()
    qc_compiled = transpile(qc, backend)
    print(type(qc_compiled))
    return backend, qc_compiled

def run_quantum(iris_params, iris_dev):
    params = iris.iris2py(iris_params)
    dev = iris.iris2py_dev(iris_dev)
    # print(params)
    backend = copy.deepcopy(params[0])
    qc_compiled = copy.deepcopy(params[1])
    print(type(qc_compiled))
    job_sim = backend.run(qc_compiled, shots=1024)
    result_sim = job_sim.result()

    counts = result_sim.get_counts(qc_compiled)
    print("Total counts for 000 and 111 are:", counts)
