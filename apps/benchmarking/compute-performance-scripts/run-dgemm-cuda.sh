...
make clean
make compute-performance-iris-profiling kernel.ptx

#exit if the last program run wasn't successful
[ $? -ne 0 ] && exit

#don't proceed if the target failed to build
if ! [ -f compute-performance-iris-profiling ] || ! [ -f kernel.ptx ] ; then
  exit
fi
...
do
    IRIS_ARCHS=cuda ./compute-performance-iris-profiling 4096 0 ${num_devices} ${REPEATS} dgemm-iris-cuda-${MACHINE}-${num_devices}.csv
done
