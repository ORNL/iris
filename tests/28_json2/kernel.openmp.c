...
static int iris_openmp_kernel0_setmem(int idx, void* mem) {
  switch (idx) {
    ...
    case 1: kernel0_args.A = (__global int *) mem; break;
    case 2: kernel0_args.B = (__global int *) mem; break;
    default: return IRIS_ERROR;
  }
  return IRIS_SUCCESS;
}
...
int iris_openmp_launch(int dim, size_t *off, size_t *ndr) {
  switch (iris_openmp_kernel_idx) {
    case 0: vecadd(kernel0_args.C, kernel0_args.A, kernel0_args.B, off, ndr); break;
  }
  iris_openmp_unlock();
  return IRIS_SUCCESS;
}
