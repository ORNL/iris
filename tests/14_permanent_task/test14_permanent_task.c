...
int main(int argc, char** argv) {
  ...
  iris_task_submit(task1, iris_minimum, NULL, true);
  ...
  for (int i = 0; i < LOOP; i++) iris_task_submit(task2, iris_minimum, NULL, true);

  iris_task task3;
  iris_task_create(&task3);
  iris_task_d2h_full(task3, mem, B);
  iris_task_submit(task3, iris_minimum, NULL, true);
